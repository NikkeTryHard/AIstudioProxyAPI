        ...
        while True:
            ...
            try:
                ...
                if data:
                    ...
                    if done and not has_content and received_items_count == 1 and not stale_done_ignored:
                ...
                if page and time_since_last_activity > POLL_INTERVAL:
                    ...
                    try:
                        ...
                        if func_call_json:
                            logger.info(f"[{req_id}] [流式] 检测到 DOM 中的函数调用，正在回退到 DOM 提取...")
                            ...
                            try:
                                ...
                                for func_idx, tc in enumerate(tool_calls_data):
                                    ...
                                    fn_name = tc.get("name") or tc.get("function_name") or "Bash"

                                    fn_args = tc.get("arguments") or tc.get("params") or tc
                                ...
                                logger.error(f"[{req_id}] Error parsing fallback JSON: {parse_err}")

                    except Exception as poll_err:
                         logger.warning(f"[{req_id}] Error during DOM polling: {poll_err}")

                    # Reset activity timer to avoid hammering DOM?
                    # No, if we found nothing, we should wait a bit.
                    # But we are in the empty loop.
                    last_queue_activity_time = time.time() # Reset to wait another interval
                ...
                if empty_count >= max_empty_retries:
                    logger.warning(f"[{req_id}] 流响应队列空读取次数达到上限 ({max_empty_retries})，且DOM轮询未获结果。")
