...
class ImageURL(BaseModel):
    url: str
    # OpenAI compatibility: detail can be 'auto' | 'low' | 'high'
...
class MessageContentItem(BaseModel):
    ...
    # Extended support for generic file_url/media_url and direct url field, keeping OpenAI style compatibility
...
class ChatCompletionRequest(BaseModel):
    ...
    attachments: Optional[List[Any]] = None
    # MCP per-request endpoint (optional), for tool call fallback to MCP service
    mcp_endpoint: Optional[str] = None
    # Dedicated system instruction field - provides clean separation from chat messages
    # Takes priority over role='system' messages if both are provided
