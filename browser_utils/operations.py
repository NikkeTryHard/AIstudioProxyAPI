...
async def _handle_model_list_response(response: Any):
    ...
    excluded_model_ids: Set[str] = getattr(server, "excluded_model_ids", set())
        ...
        try:
            ...
            if models_array_container is not None:
                ...
                for entry_in_container in models_array_container:
                    model_fields_list: Optional[Union[Dict[Any, Any], List[Any]]] = None
                ...
                if new_parsed_list:
                    ...
                    if DEBUG_LOGS_ENABLED:
                        log_output = (
                            f"成功解析和更新模型列表。总共解析模型数: {len(server.parsed_model_list)}.\n"
                        )
...
async def save_error_snapshot(
    ...
    await save_error_snapshot_legacy(error_name, error_exception)
...
async def get_response_via_function_card(page: AsyncPage, req_id: str) -> Optional[str]:
    """
    ...
    try:
        ...
        if count == 0:
            if DEBUG_LOGS_ENABLED:
                logger.debug(
                    f"[{req_id}] (FunctionScraper) No 'ms-function-call-chunk' elements found on page."
                )
            return None

        logger.info(
            f"[{req_id}] (FunctionScraper) Function call card detected! Found {count} chunks. Processing..."
        )
            ...
            try:
                ...
                if await header.count() > 0:
                    ...
                    if is_expanded == "false":
                        logger.info(
                            f"[{req_id}] (FunctionScraper) Chunk {i+1}/{count} is collapsed. Expanding..."
                        )
                        await header.click()
                        try:
                            await chunk.locator(".mat-expansion-panel-body").wait_for(
                                state="visible", timeout=2000
                            )
                            logger.info(
                                f"[{req_id}] (FunctionScraper) DEBUG: Expansion body visible."
                            )
                        except Exception as wait_err:
                            logger.warning(
                                f"[{req_id}] (FunctionScraper) Wait for expansion body failed for chunk {i+1}: {wait_err}"
                            )
                ...
                try:
                    ...
                    logger.info(
                        f"[{req_id}] (FunctionScraper) Chunk {i+1}: Extracted name='{fn_name}'"
                    )

                except Exception as name_err:
                    logger.warning(
                        f"[{req_id}] (FunctionScraper) Failed to extract function name for chunk {i+1}: {name_err}"
                    )
                ...
                if code_count > 0:
                    ...
                    if json_text:
                        try:
                            ...
                            logger.info(
                                f"[{req_id}] (FunctionScraper) Chunk {i+1}: Added tool call '{fn_name}'"
                            )

                        except json.JSONDecodeError:
                            logger.warning(
                                f"[{req_id}] (FunctionScraper) Chunk {i+1} invalid JSON content"
                            )
                else:
                    ...
                    if await alt_code.count() > 0:
                        logger.info(
                            f"[{req_id}] (FunctionScraper) DEBUG: Found ms-code-block but no 'pre code' inside?"
                        )

            except Exception as chunk_err:
                logger.warning(
                    f"[{req_id}] (FunctionScraper) Error processing chunk {i+1}: {chunk_err}"
                )

        if tool_calls:
            logger.info(
                f"[{req_id}] (FunctionScraper) Successfully extracted {len(tool_calls)} tool calls."
            )
            return json.dumps(tool_calls, ensure_ascii=False)
        else:
            if DEBUG_LOGS_ENABLED:
                logger.debug(
                    f"[{req_id}] (FunctionScraper) No valid tool calls extracted from {count} chunks."
                )
            return None
