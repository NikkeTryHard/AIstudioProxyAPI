...
process_status_text_var: tk.StringVar = tk.StringVar()
port_entry_var: tk.StringVar = tk.StringVar()
camoufox_debug_port_var: tk.StringVar = tk.StringVar()
pid_listbox_widget: Optional[tk.Listbox] = None
custom_pid_entry_var: tk.StringVar = tk.StringVar()
widgets_to_translate: List[Dict[str, Any]] = []
proxy_address_var: tk.StringVar = tk.StringVar()
proxy_enabled_var: tk.BooleanVar = tk.BooleanVar()
active_auth_file_display_var: tk.StringVar = tk.StringVar()
g_config: Dict[str, Any] = {}
stream_port_enabled_var: tk.BooleanVar = tk.BooleanVar()
stream_port_var: tk.StringVar = tk.StringVar()
helper_enabled_var: tk.BooleanVar = tk.BooleanVar()
helper_endpoint_var: tk.StringVar = tk.StringVar()
port_auto_check_var: tk.BooleanVar = tk.BooleanVar()
...
def check_all_required_ports(ports_to_check: List[Tuple[int, str]]) -> bool:
    ...
    if messagebox.askyesno(
        ...
        pids_processed_this_cycle = (
            set()
        )  # Tracks PIDs for which kill attempts (normal or admin) have been made in this call
...
def _check_llm_backend_and_launch_thread():
    ...
    try:
        with socket.create_connection(
            ("localhost", current_fastapi_port), timeout=3
        ) as sock:
            backend_ok = True
        logger.info(
        ...
        if normal_kill_success:
            ...
            if messagebox.askyesno(
                ...
                if admin_kill_success:
                    ...
                    messagebox.showwarning(
                        get_text("warning_title"),
                        get_text("admin_stop_failure", pid=pid_to_stop, error="未知错误"),
                        parent=root_widget,
                    )
            else:
                messagebox.showwarning(
        ...
        if normal_kill_success:
            ...
            if messagebox.askyesno(
                ...
                if admin_kill_success:
                    ...
                    messagebox.showwarning(
                        get_text("warning_title"),
                        get_text("admin_stop_failure", pid=pid_to_kill, error="未知错误"),
                        parent=root_widget,
                    )
            else:
                messagebox.showwarning(
...
def build_gui(root: tk.Tk):
    ...
    # Remove local re-declarations if any remain from previous edits
    # But since we use globals now, we just use them directly.
    # The previous code block for variable initialization is now replacing the local definitions.
