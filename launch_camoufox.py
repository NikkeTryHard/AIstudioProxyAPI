...
try:
    ...
    if "--internal-launch" in sys.argv or any(
        ...
        print(
            "❌ 致命错误：内部启动模式需要 'camoufox.server.launch_server' 和 'camoufox.DefaultAddons' 但无法导入。",
            file=sys.stderr,
        )
...
def cleanup():
    ...
    if camoufox_proc and camoufox_proc.poll() is None:
        ...
        try:
            ...
            logger.warning(
                f"  ⚠️ Camoufox (PID: {pid}) SIGTERM 超时。正在发送 SIGKILL 强制终止..."
            )
            if (
                sys.platform != "win32"
                and hasattr(os, "getpgid")
        ...
        logger.info(
            f"Camoufox 内部子进程 (PID: {camoufox_proc.pid if hasattr(camoufox_proc, 'pid') else 'N/A'}) 先前已自行结束，退出码: {camoufox_proc.poll()}。"
        )
        camoufox_proc = None
    else:
...
def check_dependencies():
    ...
    if launch_server is not None and DefaultAddons is not None:
        ...
        logger.warning(
            "  ⚠️ 'camoufox.server' 已导入，但 'camoufox.DefaultAddons' 未导入。排除插件功能可能受限。"
        )
    missing_py_modules = []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(f"流式代理服务器使用端口" f"提供来禁用此功能 --stream-port=0 . 默认: {DEFAULT_STREAM_PORT}"),
    )
    parser.add_argument(
        "--helper",
        type=str,
    ...
    mode_selection_group.add_argument(
        "--debug", action="store_true", help="启动调试模式 (浏览器界面可见，允许交互式认证)"
    )
    mode_selection_group.add_argument(
        "--headless", action="store_true", help="启动无头模式 (浏览器无界面，需要预先保存的认证文件)"
    )
    mode_selection_group.add_argument(
        "--virtual-display", action="store_true", help="启动无头模式并使用虚拟显示 (Xvfb, 仅限 Linux)"
    )  # from dev
    ...
    if current_system_for_camoufox == "Linux":
        ...
        logger.warning(
            f"无法识别当前系统 '{current_system_for_camoufox}'。Camoufox OS 模拟将默认设置为: {simulated_os_for_camoufox}"
        )
    logger.info(
        f"根据当前系统 '{current_system_for_camoufox}'，Camoufox OS 模拟已自动设置为: {simulated_os_for_camoufox}"
    )
    ...
    if args.internal_launch_mode:
        ...
        print(
            f"--- [内部Camoufox启动] 正在调用 camoufox.server.launch_server ... ---", flush=True
        )
    ...
    if args.debug:
        ...
        if platform.system() != "Linux":
            logger.warning(
                "⚠️ --virtual-display 模式主要为 Linux 设计。在非 Linux 系统上，其行为可能与标准无头模式相同或导致 Camoufox 内部错误。"
            )
    else:
        ...
        if user_mode_choice in valid_choices:
            ...
            logger.info(
                f"无效输入 '{user_mode_choice}' 或超时，使用默认启动模式: {final_launch_mode}模式"
            )
    ...
    if final_launch_mode == "debug" and not args.active_auth_json:
        create_new_auth_choice = (
            input_with_timeout("  是否要创建并保存新的认证文件? (y/n; 默认: n, 15s超时): ", 15)
            .strip()
            .lower()
        )
    ...
    if is_port_in_use(server_target_port, host=uvicorn_bind_host):
        ...
        if pids_on_port:
            ...
            if final_launch_mode == "debug":
                ...
                choice = (
                    input_with_timeout(
                        f"     是否尝试终止这些进程？ (y/n, 输入 n 将继续并可能导致启动失败, 15s超时): ", 15
                    )
                    .strip()
                    .lower()
                )
    ...
    if args.active_auth_json:
        ...
        if effective_active_auth_json_path:
            logger.info(
                f"  将使用通过 --active-auth-json 解析的认证文件: {effective_active_auth_json_path}"
            )
        else:
            logger.error(
                f"❌ 指定的认证文件 (--active-auth-json='{args.active_auth_json}') 未找到或不是一个文件。"
            )
            sys.exit(1)
    else:
            ...
            try:
                if os.path.exists(ACTIVE_AUTH_DIR):
                    ...
                    if active_json_files:
                        ...
                        logger.info(
                            f"  将使用 '{ACTIVE_AUTH_DIR}' 中按名称排序的第一个JSON文件: {os.path.basename(effective_active_auth_json_path)}"
                        )
                    else:
                ...
                logger.warning(
                    f"  扫描 '{ACTIVE_AUTH_DIR}' 时发生错误: {e_scan_active}", exc_info=True
                )
        ...
        if final_launch_mode == "debug" and not args.auto_save_auth:
            ...
            if available_profiles:
                ...
                choice = input_with_timeout(
                    f"   请选择要加载的认证文件编号 (输入 N 或直接回车则不加载, {args.auth_save_timeout}s超时): ",
            ...
            logger.error(
                f"  ❌ {final_launch_mode} 模式错误: --active-auth-json 未提供，且活动认证目录 '{ACTIVE_AUTH_DIR}' 中未找到任何 '.json' 认证文件。请先在调试模式下保存一个或通过参数指定。"
            )
            sys.exit(1)
    ...
    try:
        ...
        logger.info(
            f"  Camoufox 内部进程已启动 (PID: {camoufox_proc.pid})。正在等待 WebSocket 端点输出 (最长 {ENDPOINT_CAPTURE_TIMEOUT} 秒)..."
        )
        ...
        while time.time() - ws_capture_start_time < ENDPOINT_CAPTURE_TIMEOUT:
            if camoufox_proc.poll() is not None:
                logger.error(
                    f"  Camoufox 内部进程 (PID: {camoufox_proc.pid}) 在等待 WebSocket 端点期间已意外退出，退出码: {camoufox_proc.poll()}。"
                )
                break
            try:
                ...
                if line_from_camoufox is None:
                    ...
                    if camoufox_ended_streams_count >= 2:
                        logger.info(
                            f"  Camoufox 内部进程 (PID: {camoufox_proc.pid}) 的所有输出流均已关闭。"
                        )
                        break
                    continue
                ...
                if ws_match:
                    ...
                    logger.info(
                        f"  ✅ 成功从 Camoufox 内部进程捕获到 WebSocket 端点: {captured_ws_endpoint[:40]}..."
                    )
                    break
            except queue.Empty:
                continue
        ...
        if not captured_ws_endpoint and (
            ...
            logger.error(
                f"  ❌ 未能在 {ENDPOINT_CAPTURE_TIMEOUT} 秒内从 Camoufox 内部进程 (PID: {camoufox_proc.pid}) 捕获到 WebSocket 端点。"
            )
    ...
    if args.helper:  # 如果 args.helper 不是空字符串 (即 helper 功能已通过默认值或用户指定启用)
        ...
        if effective_active_auth_json_path:
            ...
            try:
                ...
                logger.warning(
                    f"    ⚠️ 无法从认证文件 '{os.path.basename(effective_active_auth_json_path)}' 加载或解析SAPISID: {e}"
                )
            ...
            if sapisid:
                ...
                logger.warning(
                    f"    ⚠️ 未能从认证文件 '{os.path.basename(effective_active_auth_json_path)}' 中找到有效的 SAPISID。HELPER_SAPISID 将不会被设置。"
                )
                if "HELPER_SAPISID" in os.environ:  # 清理，以防万一
                    del os.environ["HELPER_SAPISID"]
        else:  # args.helper 有值 (Helper 模式启用), 但没有认证文件
            logger.warning(
                f"    ⚠️ Helper 模式已启用，但没有有效的认证文件来提取 SAPISID。HELPER_SAPISID 将不会被设置。"
            )
            ...
            while not stop_watcher.is_set():
                try:
                    ...
                    if new_files:
                        ...
                        time.sleep(3)  # Allow time for file system sync
